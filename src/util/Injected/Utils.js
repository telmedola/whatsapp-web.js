    ...
    window.WWebJS.sendMessage = async (chat, content, options = {}) => {
        ...
        if (
            ...
            if (altWid) {
                ...
                sentMsg =
                    window
                        .require('WAWebCollections')
                        .Msg.get(altMsgKey._serialized || altMsgKey.$1) ||
                    window
                        .require('WAWebCollections')
                        .Chat.get(altWid)
                        ?.msgs.getModelsArray()
        ...
        if (!sentMsg) {
            window.WWebJS.lastSendMessageDebug = {
                newMsgKey: newMsgKey._serialized || newMsgKey.$1,
                chatId: chat.id._serialized || chat.id.$1,
                altWid: altWid ? altWid._serialized || altWid.$1 : null,
                lastChatMsgKeys: chat.msgs
                    .getModelsArray()
                    .slice(-3)
                    .map((m) => m.id._serialized || m.id.$1),
                time: Date.now(),
            };
        }
